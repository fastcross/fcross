...
use crate::msg::Instruction::{self, *};
use crate::state::{ChannelInfo, MY_CHANNEL, ERR_LOGS};
use crate::contract::exec::{finalize_tx, prepare_tx};
...
pub fn packet_receive_handler(
    ...
    let (attrs, msgs) = match instruction {
        Status(instr) => {
            prepare_tx(deps, &env, &instr)?
        },
        Validity(instr) => {
            finalize_tx(deps, &env, &instr)?
        },
    };
